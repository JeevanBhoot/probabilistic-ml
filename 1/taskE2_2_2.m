...
x = data.x;
y = data.y;
a = 8; % Range for input test data i.e. if a=8, range is [-8,8]
N = 100; % Number of input test data samples
input1 = 5; % Constant value of input1 (x1)
xs = [zeros(N,1)+input1, linspace(-a, a, N)']; % Test data, varying 2nd input, 1st held constant

mean_func = []; % empty - don't use mean function
cov_func = {@covSum, {@covSEard, @covSEard}}; % squared exponential covariance function
